...
const UNIVERSE_SIZE = 500; // in pixels
...
class Planet {
  ...
  updatePosition(){
    if ($('#state').html() === 'Moving'){
      ...
      [this.x, this.y] = this.getCSSCoords([this.x,this.y,this.mass]);
      console.log(` AFTER: ${this.name}: x=${this.x} y=${this.y} Vx=${this.Vx} Vy=${this.Vy} mass=${this.mass}`);  

      $(`#${this.name}`).animate( {top: `${this.y}`, left: `${this.x}`}, 0 );
    }
    setTimeout(()=>{
      this.updatePosition();
    },DELTA);    
...
$(document).ready(function(){
  ...
  $('div#universe').css('width',`${}px`);
  $('div#universe').css('height',`${}px`);
