...
class Planet {
  ...
  constructor(configObject) {
    ...
    this.dynamic = configObject.dynamic || true; // set false to create a fixed point
  ...
  updatePosition(){
    if ($('#state').html() === 'Moving'){
      ...
      console.log(`BEFORE: ${this.name}: x=${this.x} y=${this.y} Vx=${this.Vx} Vy=${this.Vy} mass=${this.mass}`);  
      const [CSSleft, CSStop] = this.getCSSCoords([this.x,this.y,this.mass]);
//      console.log(` AFTER: ${this.name}: x=${this.x} y=${this.y} Vx=${this.Vx} Vy=${this.Vy} mass=${this.mass}`);  

      $(`#${this.name}`).animate( {top: `${CSStop}`, left: `${CSSleft}`}, 0 );
    }
    if (this.dynamic){ // if dynamic is false, then no need to update positions
      setTimeout(()=>{
        this.updatePosition();
      },DELTA);  
    }
...
$(document).ready(function(){
  ...
  const y = new Planet({name:'Y', x:0, y:0, Vx:-3, Vy:-3, mass:2, color:'yellow'});
});
